...
async function uploadPdf(request, response, next) {
  try {
    if (!request.file) {
      return response.status(400).json({ erro: 'Envie um arquivo PDF no campo "arquivo".' });
    }
    ...
    // A leitura ocorre somente aqui, depois da validação do upload; nenhum arquivo é criado no disco.
